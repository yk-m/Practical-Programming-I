...
void update_cost(int node) {
	int i;
	for ( i=0; i<MAX_SIZE; i++ ) {
		if ( visited[i] )
			continue;
		if ( adjacent[node][i] == IMAX )
			continue;
		if ( cost[node] + adjacent[node][i] >= cost[i] )
			continue;

		cost[i] = cost[node] + adjacent[node][i];
		prev[i] = node;
	}
}

int select_next_node(void) {
	int i
	  , min = IMAX
	  , next = -1
	;
